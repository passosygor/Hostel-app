...
3. No campo **Database**, digite: `hotel-db` → clique em **Save**.
4. Clique com botão direito no banco `hotel-db` → **Query Tool**.
5. No editor que abrir, **copie e cole todo o conteúdo** do arquivo `hotel-db.sql`.
...
```
DATABASE_URL="postgresql://postgres:SUA_SENHA@localhost:5432/hotel-db"
```
...
```
hostel-app/
├── hotel-db.sql          ← Script SQL (banco + tabelas + dados)
├── docs/
